...
function readGamepad() {
  ...
  if (gamepad) {
    ...
    const dpadLeft = 14;
    const dpadRight = 15;
    if (gamepad.buttons[dpadLeft].pressed && fontSize > 60) fontSize -= 8; // Скорость изменения размера
    if (gamepad.buttons[dpadRight].pressed && fontSize < 1200) fontSize += 16; // Скорость изменения размера
