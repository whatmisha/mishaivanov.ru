...
function exportToSVG() {
    ...
    // Вычисляем координаты точки K1 и K7, смещение и центр первого дубликата (вверх вправо)
    const angle1Hour = ((1 * 30) - 90) * Math.PI / 180;
    const k1Point = {
        x: gridCenterX + Math.cos(angle1Hour) * 250,
        y: gridCenterY + Math.sin(angle1Hour) * 250
    };
    
    const angle7Hour = ((7 * 30) - 90) * Math.PI / 180;
    const k7Point = {
        x: gridCenterX + Math.cos(angle7Hour) * 250,
        y: gridCenterY + Math.sin(angle7Hour) * 250
    };
    ...
    // Создаем группу для модульной сетки первого дубликата
    const duplicateGridGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
    
    // Добавляем круги для первого дубликата
    for (let radius of gridRadii) {
        const circle = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
        circle.setAttribute('class', 'st1');
    ...
    // Добавляем линии от центра для первого дубликата
    for (let angle of angles) {
        const radian = angle * Math.PI / 180;
    ...
    // Вычисляем координаты точки K1 и K7 для второго дубликата (вниз влево)
    const secondAngle1Hour = ((1 * 30) - 90) * Math.PI / 180;
    const secondK1Point = {
        x: gridCenterX + Math.cos(secondAngle1Hour) * 250,
        y: gridCenterY + Math.sin(secondAngle1Hour) * 250
    };
    
    const secondAngle7Hour = ((7 * 30) - 90) * Math.PI / 180;
    const secondK7Point = {
        x: gridCenterX + Math.cos(secondAngle7Hour) * 250,
        y: gridCenterY + Math.sin(secondAngle7Hour) * 250
    };
    
    // Вычисляем смещение для второго дубликата (в противоположную сторону)
    const secondOffsetX = secondK7Point.x - secondK1Point.x;
    const secondOffsetY = secondK7Point.y - secondK1Point.y;
    
    const secondDuplicateCenter = { x: gridCenterX + secondOffsetX, y: gridCenterY + secondOffsetY };
    
    // Создаем группу для модульной сетки второго дубликата
    const secondDuplicateGridGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
    
    // Добавляем круги для второго дубликата
    for (let radius of gridRadii) {
        const circle = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
        circle.setAttribute('class', 'st1');
        circle.setAttribute('cx', secondDuplicateCenter.x);
        circle.setAttribute('cy', secondDuplicateCenter.y);
        circle.setAttribute('r', radius);
        secondDuplicateGridGroup.appendChild(circle);
    }
    
    // Добавляем линии от центра для второго дубликата
    for (let angle of angles) {
        const radian = angle * Math.PI / 180;
        const endX = secondDuplicateCenter.x + Math.cos(radian) * 250;
        const endY = secondDuplicateCenter.y + Math.sin(radian) * 250;
        
        const line = document.createElementNS('http://www.w3.org/2000/svg', 'line');
        line.setAttribute('class', 'st1');
        line.setAttribute('x1', secondDuplicateCenter.x);
        line.setAttribute('y1', secondDuplicateCenter.y);
        line.setAttribute('x2', endX);
        line.setAttribute('y2', endY);
        secondDuplicateGridGroup.appendChild(line);
    }
    
    svgElement.appendChild(secondDuplicateGridGroup);
    ...
    // Добавляем первый дубликат паттерна в SVG (вверх вправо)
    
    // Смещаем все точки для первого дубликата
    const shiftedPoints = points.map(point => ({
        x: point.x + offsetX,
        y: point.y + offsetY
    }));
    ...
    // Добавляем второй дубликат паттерна в SVG (вниз влево)
    
    // Смещаем все точки для второго дубликата
    const secondShiftedPoints = points.map(point => ({
        x: point.x + secondOffsetX,
        y: point.y + secondOffsetY
    }));
    
    // Создаем path для смещенной исходной линии со скруглением для второго дубликата
    const secondPathDuplicate1 = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    secondPathDuplicate1.setAttribute('class', 'st0');
    secondPathDuplicate1.setAttribute('d', createRoundedAnglePath(secondShiftedPoints, params.cornerRadius));
    mainGroup.appendChild(secondPathDuplicate1);
    
    // Создаем path для смещенной линии, повернутой на 120 градусов для второго дубликата
    const secondRotatedShiftedPoints1 = secondShiftedPoints.map(point => 
        rotatePoint(point.x, point.y, secondDuplicateCenter.x, secondDuplicateCenter.y, 120)
    );
    
    const secondPathDuplicate2 = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    secondPathDuplicate2.setAttribute('class', 'st0');
    secondPathDuplicate2.setAttribute('d', createRoundedAnglePath(secondRotatedShiftedPoints1, params.cornerRadius));
    mainGroup.appendChild(secondPathDuplicate2);
    
    // Создаем path для смещенной линии, повернутой на 240 градусов для второго дубликата
    const secondRotatedShiftedPoints2 = secondShiftedPoints.map(point => 
        rotatePoint(point.x, point.y, secondDuplicateCenter.x, secondDuplicateCenter.y, 240)
    );
    
    const secondPathDuplicate3 = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    secondPathDuplicate3.setAttribute('class', 'st0');
    secondPathDuplicate3.setAttribute('d', createRoundedAnglePath(secondRotatedShiftedPoints2, params.cornerRadius));
    mainGroup.appendChild(secondPathDuplicate3);
