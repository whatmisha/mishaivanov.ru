document.addEventListener('DOMContentLoaded', function() {
    ...
    function animate(currentTime) {
        ...
        if (deltaTime >= frameInterval) {
            hasActivePoints = false;  // Сбрасываем флаг в начале кадра
    ...
    canvas.addEventListener('mousemove', function(e) {
        ...
        // Вычисляем скорость движения курсора с учетом масштаба
    ...
    // Обновляем обработчики заморозки
    document.addEventListener('keydown', function(e) {
        if (e.code === 'Space') {
            e.preventDefault();
            if (!isFrozen) {
            ...
            // При разморозке восстанавливаем состояние слайдеров
