document.addEventListener('DOMContentLoaded', function() {
    ...
    const state = {
        rays: {
            isPaused: false, // Состояние паузы для Dotted Rays
            points: [],
            angles: [...BASE_ANGLES],
        ...
        plusRays: {
            isPaused: false, // Состояние паузы для Plus Rays
            crosses: [],
            angles: [...BASE_ANGLES],
            mouseX: -1000, // Значение за пределами холста
            mouseY: -1000, // Значение за пределами холста
    ...
    class Point {
        ...
        update() {
            ...
            const returnStrength = 0.00001; // Уменьшаем в 5 раз для более длительного "висения в воздухе"
            ...
            const friction = 0.998;  // Увеличиваем для более медленного затухания
            this.vx *= friction;
            this.vy *= friction;
        ...
        applyForce(cursorX, cursorY, radius, strength, isRepel) {
            ...
            if (distance < radius) {
                ...
                if (isRepel) {
                    this.vx -= Math.cos(angle) * force; // Множитель 1 как в версии 12
    ...
    // Функция отрисовки кадра для Dotted Rays
    function renderRaysFrame() {
        // Очищаем холст
        rayCtx.clearRect(0, 0, CANVAS_SIZE, CANVAS_SIZE);
        ...
        if (state.rays.points.length === 0) {
            ...
            // Инициализируем массив для сохранения состояния при паузе
            state.rays.pausedPoints = [];
        }
        
        // Если режим случайного движения активен, обновляем положение случайного курсора
        if (state.rays.isRandom && !state.rays.isPaused) {
            updateRandomPosition(state.rays);
        }
        
        // Обновляем положение точек только если не на паузе
        if (!state.rays.isPaused) {
            // Применяем силы от курсора и обновляем положение
            updatePoints(
                state.rays.points,
                state.rays.mouseX,
                state.rays.mouseY,
                parseInt(radiusInput.value),
                parseInt(strengthInput.value) * 0.15,
                gravityMode.checked
            );
            
            // Сохраняем текущие точки для отображения в паузе
            // Важно: создаем полную копию объектов Point, чтобы они имели все методы
            state.rays.pausedPoints = state.rays.points.map(point => {
                const newPoint = new Point(point.x, point.y);
                newPoint.originalX = point.originalX;
                newPoint.originalY = point.originalY;
                newPoint.offsetX = point.offsetX;
                newPoint.offsetY = point.offsetY;
                newPoint.vx = point.vx;
                newPoint.vy = point.vy;
                newPoint.active = point.active;
                newPoint.randomAngle = point.randomAngle;
                return newPoint;
            });
        }
        
        // Определяем, какие точки рисовать - текущие или сохраненные при паузе
        const pointsToDraw = state.rays.isPaused && state.rays.pausedPoints.length > 0 
            ? state.rays.pausedPoints 
            : state.rays.points;
        
        // Отрисовываем точки
        drawPoints(rayCtx, pointsToDraw, parseInt(sizeInput.value) / 2);
        
        // Отображаем индикатор паузы, если анимация приостановлена
        if (state.rays.isPaused) {
            drawPauseIndicator(rayCtx, CANVAS_SIZE, CANVAS_SIZE);
        }
    }
    ...
    // Функция отрисовки кадра для Plus Rays
    function renderPlusRaysFrame() {
        // Очищаем холст
        plusRaysCtx.clearRect(0, 0, CANVAS_SIZE, CANVAS_SIZE);
        ...
        if (state.plusRays.crosses.length === 0) {
            ...
            // Инициализируем массив для сохранения состояния при паузе
            state.plusRays.pausedCrosses = [];
        }
        
        // Если режим случайного движения активен, обновляем положение случайного курсора
        if (state.plusRays.isRandom && !state.plusRays.isPaused) {
            updateRandomPosition(state.plusRays);
        }
        
        // Обновляем положение крестов только если не на паузе
        if (!state.plusRays.isPaused) {
            // Применяем силы от курсора и обновляем положение
            updateCrosses(
                state.plusRays.crosses,
                state.plusRays.mouseX,
                state.plusRays.mouseY,
                parseInt(plusRaysRadiusInput.value),
                parseInt(plusRaysStrengthInput.value) * 0.15, // Еще больше уменьшаем силу воздействия
                plusRaysGravityMode.checked
            );
            
            // Сохраняем текущие кресты для отображения в паузе
            // Важно: создаем полную копию объектов Cross, чтобы они имели все методы
            state.plusRays.pausedCrosses = state.plusRays.crosses.map(cross => {
                const newCross = new Cross(cross.x, cross.y);
                newCross.originalX = cross.originalX;
                newCross.originalY = cross.originalY;
                newCross.vx = cross.vx;
                newCross.vy = cross.vy;
                newCross.active = cross.active;
                newCross.randomAngle = cross.randomAngle;
                newCross.offsetX = cross.offsetX;
                newCross.offsetY = cross.offsetY;
                return newCross;
            });
        }
        
        // Определяем, какие кресты рисовать - текущие или сохраненные при паузе
        const crossesToDraw = state.plusRays.isPaused && state.plusRays.pausedCrosses.length > 0 
            ? state.plusRays.pausedCrosses 
            : state.plusRays.crosses;
        ...
        drawCrosses(
            plusRaysCtx,
            crossesToDraw,
            parseInt(plusRaysLengthInput.value),
            parseFloat(plusRaysThicknessInput.value)
        );
        
        // Отображаем индикатор паузы, если анимация приостановлена
        if (state.plusRays.isPaused) {
            drawPauseIndicator(plusRaysCtx, CANVAS_SIZE, CANVAS_SIZE);
        }
    }
    ...
    // Функция для переключения состояния паузы для текущей активной вкладки
    function togglePause() {
        // Определяем, какая вкладка активна
        const dottedRaysActive = document.getElementById('dotted-rays-content').classList.contains('active');
        const plusRaysActive = document.getElementById('plus-rays-content').classList.contains('active');
        
        // Переключаем паузу только для активной вкладки
        if (dottedRaysActive) {
            state.rays.isPaused = !state.rays.isPaused;
            
            // Сохраняем текущее состояние точек при включении паузы
            if (state.rays.isPaused) {
                state.rays.pausedPoints = state.rays.points.map(point => {
                    const newPoint = new Point(point.x, point.y);
                    newPoint.originalX = point.originalX;
            ...
            // Визуальная индикация состояния паузы
            if (rayCanvas) {
                rayCanvas.style.cursor = state.rays.isPaused ? 'not-allowed' : 'default';
            }
        } else if (plusRaysActive) {
            state.plusRays.isPaused = !state.plusRays.isPaused;
            
            // Сохраняем текущее состояние крестов при включении паузы с сохранением метода draw
            if (state.plusRays.isPaused) {
                state.plusRays.pausedCrosses = state.plusRays.crosses.map(cross => {
                    const newCross = new Cross(cross.x, cross.y);
                    newCross.originalX = cross.originalX;
            ...
            // Визуальная индикация состояния паузы
            if (plusRaysCanvas) {
                plusRaysCanvas.style.cursor = state.plusRays.isPaused ? 'not-allowed' : 'default';
            }
        } else {
            // Для Grid используется отдельная логика паузы в grid.js
            const event = new CustomEvent('grid-toggle-pause');
            window.dispatchEvent(event);
        }
    }
    ...
    // Обработчик клика по канвасам
    [rayCanvas, plusRaysCanvas].forEach(canvas => {
        if (canvas) {
    ...
    // Добавляем обработчик для канваса сетки после загрузки DOM
...
// Функция для отображения индикатора паузы на холсте
function drawPauseIndicator(ctx, canvasWidth, canvasHeight) {
    // Полупрозрачный фон
...
// Делаем функцию drawPauseIndicator глобально доступной для других скриптов
window.drawPauseIndicator = drawPauseIndicator; 