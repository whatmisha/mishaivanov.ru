...
export class PresetsController {
    ...
    async initPresets() {
        ...
        presetDropdownMenu.addEventListener('click', async (e) => {
            ...
            if (item) {
                ...
                if (presetName === '__restore_defaults__') {
                    presetDropdownToggle.setAttribute('aria-expanded', 'false');
                    presetDropdownMenu.classList.remove('active');
                    this.resetPresetDropdownMenuScroll(presetDropdownMenu);

                    const confirmed = await app.modalManager.confirmRestoreDefaults();
                    if (confirmed) {
                        await this.restoreDefaultPresets();
                    }
                    return;
                }
                ...
                if (presetName && (presetName !== app.currentPresetName || wantsResetToNew)) {
                    ...
                    const leavingEphemeralShare = app.currentPresetName === '__shared__';
                    if (app.hasUnsavedChanges || leavingEphemeralShare) {
                        presetDropdownToggle.setAttribute('aria-expanded', 'false');
                        presetDropdownMenu.classList.remove('active');
                        this.resetPresetDropdownMenuScroll(presetDropdownMenu);

                        const presetNameForDialog = this._presetUnsavedSwitchDialogLabel(app);
                        const action = await app.modalManager.confirmUnsavedChanges(presetNameForDialog);

                        if (action === 'cancel') return;

                        if (action === 'save') {
                            if (app.currentPresetName === 'New' || app.currentPresetName === '__shared__') {
        ...
        const presetToolbarShareBtn = document.getElementById('presetToolbarShareBtn');
        if (presetToolbarShareBtn) {
            presetToolbarShareBtn.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                void this.sharePresetToClipboard(app.currentPresetName || 'New');
            });
        }
    ...
    /** Label shown in confirmUnsavedChanges when switching away from preset. */
    _presetUnsavedSwitchDialogLabel(app) {
        const n = app.currentPresetName;
        if (n === 'New') return 'New';
        if (n === '__shared__') {
            const s = String(app.sharedPresetSuggestedName || '').trim();
            return s || 'Unsaved*';
        }
        return n;
    }

    /** Remove legacy `#sharedPresetBanner` node if present (sharing UI was removed). */
    removeStaleSharedBanner() {
        document.getElementById('sharedPresetBanner')?.remove();
    }
    ...
    getPresetToggleButtonLabel() {
        const app = this.app;
        const dirty = !!app.hasUnsavedChanges;
        if (app.currentPresetName === '__shared__') {
            const suggested = String(app.sharedPresetSuggestedName || '').trim();
            if (suggested) {
                const base = this.getDisplayName(suggested);
                return dirty ? `${base} *` : base;
            }
            return 'Unsaved*';
        }
        if (app.currentPresetName === 'New' && dirty) return 'Unsaved*';
        const base = this.getPresetListItemLabel(app.currentPresetName || 'New');
        if (dirty && app.currentPresetName && app.currentPresetName !== 'New') {
            return `${base} *`;
        }
        return base;
    }
    ...
    /**
     * Ensure the `New` preset exists in storage (created from current settings).
     * Used during init and after Restore-default-presets so the dropdown
     * always has a baseline entry.
     */
    ensureDefaultNewPreset() {
        const app = this.app;
        if (app.presetManager.loadPreset('New')) return;
        const presetData = this.collectPresetData({ propagateSeeded: false });
        app.presetManager.savePreset('New', presetData);
    }

    /**
     * Wipe every persisted preset and reseed the shipped library, then
     * land the user on a clean `New`. Used by the "Restore default presets"
     * dropdown action and reachable only after explicit confirmation.
     */
    async restoreDefaultPresets() {
        const app = this.app;
        // Drop in-session caches first so a later loadPreset can't restore
        // a snapshot belonging to a preset we are about to wipe.
        app.presetHistories.clear();
        app.historyManager = null;
        app.currentPresetName = null;
        app.hasUnsavedChanges = false;
        app.sharedPresetSuggestedName = '';
        this.removeStaleSharedBanner();

        try {
            await app.presetManager.clearAllAndReseed(
                app.pristineSettingsDefaults || null,
                'presets/'
            );
        } catch (e) {
            console.warn('[presets] restoreDefaultPresets failed', e);
        }

        // Recreate the `New` slot from pristine defaults so the dropdown has
        // a baseline + a sane place to land after the reset.
        if (app.pristineSettingsDefaults) {
            Object.assign(app.settings.values, app.pristineSettingsDefaults);
        }
        this.ensureDefaultNewPreset();
    ...
    updateSaveDeleteButtons() {
        ...
        if (currentMode === 'editor') {
            ...
            if (presetToolbarShareBtn) presetToolbarShareBtn.style.display = 'none';
            return;
        }
        ...
        const showShareToolbar =
            !(app.currentPresetName === 'New' && !app.hasUnsavedChanges);
        if (savePresetBtn) {
            savePresetBtn.style.display =
                app.hasUnsavedChanges || app.currentPresetName === '__shared__'
                    ? 'inline-flex'
                    : 'none';
        }

        if (presetToolbarShareBtn) {
            presetToolbarShareBtn.style.display = showShareToolbar ? 'inline-flex' : 'none';
        }
    ...
    gatherPresetBlobForShare(presetName) {
        ...
        delete raw.id;
        if (presetName === '__shared__') {
            const sug = String(app.sharedPresetSuggestedName || '').trim();
            if (sug) {
                raw.shareDisplayName = sug;
            } else {
                delete raw.shareDisplayName;
            }
        } else if (presetName === 'New') {
            const line = String(raw.text ?? '')
                .split('\n')[0]
                .trim();
            raw.shareDisplayName = line.slice(0, 80) || 'New';
        } else {
    ...
    async sharePresetToClipboard(presetName) {
        ...
        try {
            result = await encodePresetShareWithBudget(blob, pristine, {
                urlPrefix,
                maxUrlChars: SHARE_SOFT_LIMIT_CHARS
            });
        } catch (e) {
        ...
        if (!result || !result.encoded) {
            await app.modalManager.showError('Could not build share link.');
            return;
        }
        const url = `${urlPrefix}${result.encoded}`;

        try {
            if (typeof localStorage !== 'undefined' && localStorage.getItem('voidShareDebug')) {
                console.debug(
                    '[share] URL chars', url.length,
                    'payload token chars', result.encoded.length,
                    'tier', result.tier,
                    'fits', result.fits
                );
            }
    ...
    async applySharedPresetFromEncodedFragment(encoded) {
        ...
        const { full, shareDisplayName = '' } = decoded;
        delete full.seeded;
        app.sharedPresetSuggestedName = String(shareDisplayName || '').trim();
