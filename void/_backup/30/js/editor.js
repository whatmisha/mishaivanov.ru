...
class GlyphEditorApp {
    ...
    deleteCurrentGlyph() {
        ...
        if (editedGlyphs[this.selectedChar] && editedGlyphs[this.selectedChar][key]) {
            ...
            const shouldDeleteChar = Object.keys(editedGlyphs[this.selectedChar]).length === 0;
            if (shouldDeleteChar) {
                ...
                const importedChars = this.getImportedCharList();
                if (importedChars) {
                    const index = importedChars.indexOf(this.selectedChar);
                    if (index > -1) {
            ...
            // Если символ был полностью удалён
            if (shouldDeleteChar) {
                // Сбросить выбор
                this.selectedChar = null;
                this.selectedAlternativeIndex = null;
    ...
    generateFullVoidAlphabetFile(editedGlyphs) {
        // Заголовок (строки 1-18)
        let code = '/**\n';
        code += ' * VoidAlphabet - данные модульного шрифта Void\n';
        code += ' * \n';
        code += ' * Каждый глиф закодирован строкой из 50 символов:\n';
        code += ' * - 25 модулей на сетке 5×5\n';
        code += ' * - Каждый модуль = буква + цифра (тип + поворот)\n';
        code += ' * \n';
        code += ' * Типы модулей:\n';
        code += ' * S — (straight) вертикальная линия слева\n';
        code += ' * R — (rounded) плавная дуга, радиус = размер модуля\n';
        code += ' * B — (bend) крутая дуга, радиус = половина модуля\n';
        code += ' * C — (central) вертикальная линия по центру\n';
        code += ' * J — (joint) Т-образное соединение\n';
        code += ' * L — (corner) L-образное соединение\n';
        code += ' * E — (empty) пустая ячейка\n';
        code += ' * \n';
        code += ' * Повороты: 0 = 0°, 1 = 90°, 2 = 180°, 3 = 270°\n';
        code += ' */\n\n';
        ...
        if (latin.length > 0) {
            ...
            latin.forEach((char, i) => {
                const comma = (i < latin.length - 1 || digits.length > 0 || symbols.length > 0 || cyrillic.length > 0) ? ',' : '';
        ...
        if (digits.length > 0) {
            ...
            digits.forEach((char, i) => {
                const comma = (i < digits.length - 1 || symbols.length > 0 || cyrillic.length > 0) ? ',' : '';
                code += `    "${char}": "${allBaseGlyphs[char]}"${comma}\n`;
            });
            code += '    \n';
        }
        ...
        if (symbols.length > 0) {
            ...
            symbols.forEach((char, i) => {
                const comma = (i < symbols.length - 1 || cyrillic.length > 0) ? ',' : '';
                code += `    "${char}": "${allBaseGlyphs[char]}"${comma}\n`;
            });
            code += '    \n';
        }
        ...
        if (cyrillic.length > 0) {
            ...
            cyrillic.forEach((char, i) => {
                const comma = i < cyrillic.length - 1 ? ',' : '';
                code += `    "${char}": "${allBaseGlyphs[char]}"${comma}\n`;
            });
        }
        ...
        code += '/**\n';
        code += ' * Альтернативные глифы для символов\n';
        code += ' * Ключ - символ, значение - массив альтернативных глифов\n';
        code += ' */\n';
        ...
        // Функции
        code += '/**\n';
        code += ' * Получить код глифа для символа\n';
        code += ' */\n';
        ...
        code += '    \n';
        code += '    // Если индекс не указан - возвращаем базовый глиф\n';
        code += '    if (alternativeIndex === null || alternativeIndex === undefined) {\n';
        code += '        return baseGlyph;\n';
        code += '    }\n';
        code += '    \n';
        code += '    // Режим random: выбираем случайную альтернативу\n';
        ...
        code += '/**\n';
        code += ' * Проверить, есть ли глиф для символа\n';
        code += ' * @param {string} char - символ\n';
        code += ' * @returns {boolean}\n';
        code += ' */\n';
        code += 'export function hasGlyph(char) {\n';
        code += '    return VOID_ALPHABET.hasOwnProperty(char.toUpperCase());\n';
        code += '}\n\n';
        
        code += '/**\n';
        code += ' * Получить список всех доступных символов\n';
        code += ' * @returns {string[]}\n';
        code += ' */\n';
        code += 'export function getAvailableChars() {\n';
        code += '    return Object.keys(VOID_ALPHABET);\n';
        code += '}\n\n';
