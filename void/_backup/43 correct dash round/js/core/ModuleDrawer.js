    ...
    drawStraight(ctx, x, y, w, h, angle, stem) {
        ...
        if (this.renderMethod === 'stroke') {
            ...
            if (this.mode === 'fill') {
                ...
                const shortenTopStripes = this.roundedCaps && localEndpoints && localEndpoints.top ? strokeWidth / 2 : 0;
                const shortenBottomStripes = this.roundedCaps && localEndpoints && localEndpoints.bottom ? strokeWidth / 2 : 0;
                ...
                if (this.closeEnds && localEndpoints) {
                    ...
                    ctx.lineCap = 'butt'; // Закрывающие линии всегда с butt cap
                ...
                // Это гарантирует, что паттерн штрихов одинаков независимо от roundedCaps
                const lineLength = h;
                ...
                // Сдвигаем паттерн, чтобы штрихи были на тех же позициях, что и при полной линии
                ctx.lineDashOffset = -shortenTop;
                ...
                ctx.lineDashOffset = 0; // Сброс offset
            }
        } else {
    ...
    drawCentral(ctx, x, y, w, h, angle, stem) {
        ...
        if (this.renderMethod === 'stroke') {
            ...
            if (this.mode === 'fill') {
                ...
                ctx.setLineDash([adaptive.dashLength, adaptive.gapLength]);
                // Сдвигаем паттерн, чтобы штрихи были на тех же позициях
                ctx.lineDashOffset = -shortenTop;
    ...
    drawJoint(ctx, x, y, w, h, angle, stem) {
        ...
        if (this.renderMethod === 'stroke') {
            ...
            if (this.mode === 'fill') {
                ...
                // Укорачивание для горизонтальной линии (если включен roundedCaps и есть endpoints)
                ...
                // Вертикальная линия: вычисляем пунктир для ПОЛНОЙ длины
                const vertLength = h;
                ...
                // Горизонтальная линия: вычисляем пунктир для ПОЛНОЙ длины
                // Начинается от вертикальной линии (vertLineX), заканчивается на w/2
                const horizStartX = vertLineX;
                const horizEndX = w / 2 - shortenRight;
                const horizFullLength = w / 2 - vertLineX; // Полная длина без укорачивания
                const horizAdaptive = this.calculateAdaptiveDash(horizFullLength, dashPx, gapPx);
                
                ctx.setLineDash([horizAdaptive.dashLength, horizAdaptive.gapLength]);
                ctx.lineDashOffset = 0; // Горизонтальная линия начинается без смещения (укорачивание только справа)
                ctx.beginPath();
    ...
    drawLink(ctx, x, y, w, h, angle, stem) {
        ...
        if (this.renderMethod === 'stroke') {
            ...
            if (this.mode === 'fill') {
                ...
                // Укорачивание (если включен roundedCaps или closeEnds, и есть endpoints)
                const shouldShorten = (this.roundedCaps || this.closeEnds) && localEndpoints;
                const shortenTop = shouldShorten && localEndpoints.top ? stem * 0.25 : 0;
                const shortenRight = shouldShorten && localEndpoints.right ? stem * 0.25 : 0;
                const shortenBottom = shouldShorten && localEndpoints.bottom ? stem * 0.25 : 0;
                ...
                // Для L-образного соединения вычисляем ПОЛНУЮ длину пути (без укорачивания)
                // Вертикальная часть: от верха до начала горизонтальной части
                const vertFullLength = h / 2 + horizLineY;
                
                // Горизонтальная часть: от вертикальной линии до правого края
                const horizFullLength = w / 2 - vertLineX;
                
                const totalFullLength = vertFullLength + horizFullLength;
                
                // Вычисляем адаптивный dash для ПОЛНОЙ длины пути
                const adaptive = this.calculateAdaptiveDash(totalFullLength, dashPx, gapPx);
                
                ctx.setLineDash([adaptive.dashLength, adaptive.gapLength]);
                // Сдвигаем паттерн на величину укорачивания сверху
                ctx.lineDashOffset = -shortenTop;
        ...
        if (this.renderMethod === 'stroke') {
            ...
            if (this.mode === 'fill') {
                ...
                if (this.closeEnds && localEndpoints && this.strokesNum > 0) {
                    ...
                    // Радиусы первой (внешней) и последней (внутренней) дуги
                    const firstRadius = outerRadius;
                    let lastRadius = outerRadius - (this.strokesNum - 1) * (strokeWidth + gap);
                    
                    // Защита: если lastRadius слишком мал, используем minRadius
                    if (lastRadius < minRadius) {
                        lastRadius = minRadius;
                    }
                    
                    ctx.lineCap = 'round'; // Закрывающие линии скругляются на концах
                    
                    // Закрывающая линия на right (startAngle = PI/2)
                    if (localEndpoints.right) {
                        // Учитываем укорачивание дуг: вычисляем deltaAngle для каждого радиуса
                    ...
                    // Закрывающая линия на top (endAngle = PI)
                    if (localEndpoints.top) {
                        // Учитываем укорачивание дуг: вычисляем deltaAngle для каждого радиуса
                        const deltaAngleFirst = shortenAmount / firstRadius;
                        const deltaAngleLast = shortenAmount / lastRadius;
                ...
                // Вычисляем длину дуги для ПОЛНОЙ дуги (без укорачивания): L = radius * angle
                const fullArcAngle = Math.PI / 2; // От PI/2 до PI
                const fullArcLength = arcRadius * fullArcAngle;
                const adaptive = this.calculateAdaptiveDash(fullArcLength, dashPx, gapPx);
                
                ctx.setLineDash([adaptive.dashLength, adaptive.gapLength]);
                // Сдвигаем паттерн на величину укорачивания справа (в единицах длины дуги)
                ctx.lineDashOffset = -shortenAmount;
